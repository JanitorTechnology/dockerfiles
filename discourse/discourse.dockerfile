...
RUN sudo sed -i "s/-w \/home\/user/-w \/home\/user\/discourse/" /etc/supervisord.conf && \
    printf "export DISCOURSE_PORT=\"3000\"" >> /home/user/.bashrc && \
    printf "export DISCOURSE_RELATIVE_URL_ROOT=/`cat /proc/self/cgroup | grep docker | head -n 1 | sed 's_^.*docker\/__'`/\$DISCOURSE_PORT" >> /home/user/.bashrc
